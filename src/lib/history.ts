...
import { codeHashes, DEPLOYMENT, readOnlyClient } from "./chain";
...
async function codeHashAt(): Promise<(height: number) => string> {
  // The current hash, whatever it turns out to be, standing in for any height whose code
  // cannot be established. Resolved once here rather than read from a constant, because
  // there is no longer a constant to read.
  const { core: current } = await codeHashes();
  ...
  try {
    ...
    return (height: number) => {
      ...
      return hashes.get(chosen.codeId) ?? current;
    };
  } catch {
    return fallback;
  }
